 ...
 *
 * MT8188 capture sample: UL9 ← CM0 (bypass) ← AFE_CONN ← eTDM_IN1 + eTDM_IN2,
...
int main(void)
{
	...
	ret = api->configure(dev, MT8188_UL9, &cfg);
	if (ret) {
	...
	ret = api->route(dev, MT8188_ROUTE_SRC_ETDM_IN1_IN2, MT8188_ROUTE_DST_UL9);
	if (ret) {
	...
	ret = api->set_buf(dev, MT8188_UL9, dma_buf_pa, BUF_BYTES);
	if (ret) {
		printk("set_buf failed: %d\n", ret);
		return ret;
	}

	ret = api->start(dev, MT8188_UL9);
	if (ret) {
		printk("start failed: %d\n", ret);
		return ret;
	}

	printk("MT8188 UL9 capture started: 48kHz 32ch 32-bit via CM0 cowork\n");

	while (1) {
		k_msleep(1000);

		/* get_cur returns a physical address */
		uint32_t ptr = api->get_cur(dev, MT8188_UL9);
