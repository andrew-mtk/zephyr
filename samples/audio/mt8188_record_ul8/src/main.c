...
int main(void)
{
	...
	ret = api->configure(dev, MT8188_UL8, &cfg);
	if (ret) {
		printk("configure failed: %d\n", ret);
		return ret;
	}

	ret = api->route(dev, MT8188_ROUTE_SRC_ETDM_IN1, MT8188_ROUTE_DST_UL8);
	if (ret) {
	...
	ret = api->set_buf(dev, MT8188_UL8, dma_buf_pa, BUF_BYTES);
	if (ret) {
		printk("set_buf failed: %d\n", ret);
		return ret;
	}

	ret = api->start(dev, MT8188_UL8);
	if (ret) {
	...
	while (1) {
		...
		uint32_t ptr = api->get_cur(dev, MT8188_UL8);
