...
static const uint32_t valid_baudrates [] = {
    50, 75, 110, 134, 150, 200, 300, 600, 1200, 1800, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400, 460800, 500000, 576000, 921600, 1000000, 1152000, 1500000, 2000000
};

static const size_t valid_baudrates_size = ARRAY_SIZE(valid_baudrates);

static const uint8_t fraction_L_map [] = {
    0, 1, 5, 21, 85, 87, 87, 119, 127, 255, 255
};

static const uint8_t fraction_M_map [] = {
...
static int set_baudrate(const struct device *dev, const struct uart_config *cfg)
{
    ...
    while ((idx < valid_baudrates_size)              &&
           (cfg->baudrate != valid_baudrates [idx])) {
        idx++;
    }
    ...
    else
    {
		...
		sys_write32(((sample_count >> 1) - 1), (DEVICE_MMIO_GET(dev) + UART_OFFSET_SAMPLE_POINT));

		sys_write32((uint32_t) fraction_L_map [fraction], (DEVICE_MMIO_GET(dev) + UART_OFFSET_FRACDIV_L));
		sys_write32((uint32_t) fraction_M_map [fraction], (DEVICE_MMIO_GET(dev) + UART_OFFSET_FRACDIV_M));
    }
