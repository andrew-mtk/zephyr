 ...
 */
static void mt8188_afe_tuner_enable(struct mt8188_afe *afe,
				    const struct mt8188_afe_tuner_cfg *cfg)
{
	uint32_t val;
...
static void mt8188_afe_tuner_disable(struct mt8188_afe *afe,
				     const struct mt8188_afe_tuner_cfg *cfg)
{
	sys_write32(sys_read32(afe->base + cfg->tuner_en_reg) &
		    ~BIT(cfg->tuner_en_shift),
 ...
 *
 * Owned by the caller (start()/stop()), not by the per-port helpers: a stream
 * has ONE timing domain but may drive TWO ports (UL9 = IN1+IN2, DL11 32ch =
 * OUT1+OUT2), so the domain is enabled once and the port clocks per port.
 */
int mt8188_afe_enable_apll_domain(struct mt8188_afe *afe, uint32_t rate)
{
	if (rate_uses_apll1(rate)) {
		 ...
		 * A1SYS. mt8188_apll1_enable() subsumes the A1SYS_HP gate.
		 */
...
static const struct etdm_port_clks etdm_port_clk_table[MT8188_ETDM_NR] = {
	[MT8188_ETDM_OUT1] = { CLK_TOP_APLL12_CK_DIV2, CLK_TOP_I2SO1,
			       MT8188_CLK_AUD_ETDM_OUT1 },
	[MT8188_ETDM_OUT2] = { CLK_TOP_APLL12_CK_DIV3, CLK_TOP_I2SO2,
			       MT8188_CLK_AUD_ETDM_OUT2 },
	[MT8188_ETDM_IN1]  = { CLK_TOP_APLL12_CK_DIV0, CLK_TOP_I2SI1,
			       MT8188_CLK_AUD_ETDM_IN1 },
	[MT8188_ETDM_IN2]  = { CLK_TOP_APLL12_CK_DIV1, CLK_TOP_I2SI2,
			       MT8188_CLK_AUD_ETDM_IN2 },
};
...
int mt8188_afe_enable_etdm_clocks(struct mt8188_afe *afe, enum mt8188_etdm_id id)
{
	...
	CLK_ON_TOLERANT(afe->clk_topckgen, p->div_clk);
	CLK_ON_TOLERANT(afe->clk_topckgen, p->mux_clk);
...
int mt8188_afe_disable_etdm_clocks(struct mt8188_afe *afe, enum mt8188_etdm_id id)
{
	...
	CLK_OFF_TOLERANT(afe->clk_topckgen, p->mux_clk);
	CLK_OFF_TOLERANT(afe->clk_topckgen, p->div_clk);
