...
{
	switch (id) {
	case MT8188_UL9:  return MT8188_CM0;
	case MT8188_UL2:  return MT8188_CM1;
	case MT8188_UL10: return MT8188_CM2;
	default:          return -1;
	}
}

/* mt8188_afe_config_cm() — write sel, ch_num, update_cnt */
static void cm_config(uintptr_t base, const struct mt8188_cm_data *cm,
		      uint32_t channels)
{
	uint32_t val;
...
static void cm_enable(uintptr_t base, const struct mt8188_cm_data *cm,
		      bool enable)
{
 ...
 */
static enum mt8188_etdm_id memif_second_etdm(enum mt8188_memif_id id,
					     uint32_t channels)
{
...
{
	switch (id) {
	case MT8188_DL2:   return MT8188_CLK_AUD_MEMIF_DL2;
	case MT8188_DL3:   return MT8188_CLK_AUD_MEMIF_DL3;
	case MT8188_DL6:   return MT8188_CLK_AUD_MEMIF_DL6;
	case MT8188_DL7:   return MT8188_CLK_AUD_MEMIF_DL7;
	case MT8188_DL8:   return MT8188_CLK_AUD_MEMIF_DL8;
	case MT8188_DL10:  return MT8188_CLK_AUD_MEMIF_DL10;
	case MT8188_DL11:  return MT8188_CLK_AUD_MEMIF_DL11;
	case MT8188_UL1:   return MT8188_CLK_AUD_MEMIF_UL1;
	case MT8188_UL2:   return MT8188_CLK_AUD_MEMIF_UL2;
	case MT8188_UL3:   return MT8188_CLK_AUD_MEMIF_UL3;
	case MT8188_UL4:   return MT8188_CLK_AUD_MEMIF_UL4;
	case MT8188_UL5:   return MT8188_CLK_AUD_MEMIF_UL5;
	case MT8188_UL6:   return MT8188_CLK_AUD_MEMIF_UL6;
	case MT8188_UL8:   return MT8188_CLK_AUD_MEMIF_UL8;
	case MT8188_UL9:   return MT8188_CLK_AUD_MEMIF_UL9;
	case MT8188_UL10:  return MT8188_CLK_AUD_MEMIF_UL10;
	default:           return -1;
	}
}
...
static void afe_irq_set_fs(uintptr_t base, const struct mt8188_irq_data *irq,
		       uint32_t fs)
{
	if (irq->irq_fs_reg < 0) {
		return;
 ...
 * -------------------------------------------------------------------------
 */
static int mt8188_afe_api_configure(const struct device *dev,
				    enum mt8188_memif_id memif_id,
				    const struct mt8188_afe_cfg *cfg)
{
	 ...
	 */
	{
		...
		if (slave < MT8188_ETDM_NR) {
			mt8188_etdm_set_cowork_source(afe, slave,
						      memif_to_etdm(memif_id));
	...
	if (memif_id == MT8188_UL3 ||
		...
		mt8188_etdm_set_cowork_source(afe, MT8188_ETDM_IN2,
					      MT8188_COWORK_SOURCE_NONE);
	...
	} else if (memif_id == MT8188_DL11 &&
		   memif_is_dual_port(memif_id, cfg->channels)) {
	...
	if (memif_id == MT8188_DL8 ||
		...
		mt8188_etdm_set_cowork_source(afe, MT8188_ETDM_OUT2,
					      MT8188_COWORK_SOURCE_NONE);
	...
	if (etdm_id < MT8188_ETDM_NR) {
		...
		const enum mt8188_etdm_id slave_etdm =
			memif_second_etdm(memif_id, cfg->channels);
		int ret2;
		...
		mt8188_etdm_set_data_mode(afe, etdm_id, cfg->data_mode);
		mt8188_etdm_set_tdm_slot(afe, etdm_id, cfg->slots, cfg->lrck_width);

		ret2 = mt8188_etdm_set_sysclk(afe, etdm_id, cfg->mclk_freq,
					      cfg->mclk_dir);
		if (ret2) {
			return ret2;
		}
		 ...
		 */
		if (slave_etdm < MT8188_ETDM_NR) {
			mt8188_etdm_set_fmt(afe, slave_etdm, cfg->fmt,
					    false, false, true);
			mt8188_etdm_set_data_mode(afe, slave_etdm, cfg->data_mode);
			mt8188_etdm_set_tdm_slot(afe, slave_etdm, cfg->slots,
						 cfg->lrck_width);
			ret2 = mt8188_etdm_set_sysclk(afe, slave_etdm,
 ...
 * -------------------------------------------------------------------------
 */
static int mt8188_afe_api_route(const struct device *dev,
				enum mt8188_route_src src,
				enum mt8188_route_dst dst)
{
	struct mt8188_afe *afe = dev->data;

	return mt8188_afe_route(afe, src, dst);
}
 ...
 * -------------------------------------------------------------------------
 */
static int mt8188_afe_api_start(const struct device *dev,
				enum mt8188_memif_id memif_id)
{
	struct mt8188_afe *afe = dev->data;
	const struct mt8188_memif_data *m;
	...
	/* Enable path clocks: one rate-selected APLL timing domain for the
	 * stream, then the per-port clocks for each eTDM port it drives (two
	 * for the cowork paths).
	 */
	...
	/* Ungate this memif's AUDSYS clock (memif-owned, not port-owned). */
	{
		...
		if (gate >= 0) {
			mt8188_audsys_clk_on(afe->base,
					     (enum mt8188_audsys_clk_id)gate);
		}
	}
	...
	if (m->irq_id >= 0 && m->irq_id < MT8188_IRQ_NR) {
		 ...
		 */
		if (memif_id >= MT8188_UL1 && memif_id <= MT8188_UL10 &&
 ...
 * -------------------------------------------------------------------------
 */
static int mt8188_afe_api_stop(const struct device *dev,
			       enum mt8188_memif_id memif_id)
{
	struct mt8188_afe *afe = dev->data;
	const struct mt8188_memif_data *m;
	...
	/* Gate this memif's AUDSYS clock (memif-owned, not port-owned). */
	{
		...
		if (gate >= 0) {
			mt8188_audsys_clk_off(afe->base,
					      (enum mt8188_audsys_clk_id)gate);
		}
	}
	...
	mt8188_afe_disable_apll_domain(afe, afe->memif_rt[memif_id].rate);
 ...
 * -------------------------------------------------------------------------
 */
static int mt8188_afe_api_set_buf(const struct device *dev,
				  enum mt8188_memif_id memif_id,
...
static uint32_t mt8188_afe_api_get_cur(const struct device *dev,
				       enum mt8188_memif_id memif_id)
{
	struct mt8188_afe *afe = dev->data;
	const struct mt8188_memif_data *m;
 ...
 * -------------------------------------------------------------------------
 */
static int mt8188_afe_api_set_period_cb(const struct device *dev,
					enum mt8188_memif_id memif_id,
...
static const struct mt8188_afe_driver_api mt8188_afe_api = {
...
static int mt8188_afe_init(const struct device *dev)
{
	...
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
		    mt8188_afe_isr, DEVICE_DT_INST_GET(0), 0);
...
DEVICE_DT_INST_DEFINE(0, mt8188_afe_init, NULL,
