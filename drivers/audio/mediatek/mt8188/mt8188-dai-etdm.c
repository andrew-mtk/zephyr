...
static int etdm_cowork_slv_sel(enum mt8188_etdm_id id, bool slave_mode)
{
	if (slave_mode) {
		switch (id) {
		case MT8188_ETDM_IN1:  return MT8188_COWORK_ETDM_IN1_S;
		case MT8188_ETDM_IN2:  return MT8188_COWORK_ETDM_IN2_S;
		case MT8188_ETDM_OUT1: return MT8188_COWORK_ETDM_OUT1_S;
		case MT8188_ETDM_OUT2: return MT8188_COWORK_ETDM_OUT2_S;
		default:               return -EINVAL;
		}
	} else {
		switch (id) {
		case MT8188_ETDM_IN1:  return MT8188_COWORK_ETDM_IN1_M;
		case MT8188_ETDM_IN2:  return MT8188_COWORK_ETDM_IN2_M;
		case MT8188_ETDM_OUT1: return MT8188_COWORK_ETDM_OUT1_M;
		case MT8188_ETDM_OUT2: return MT8188_COWORK_ETDM_OUT2_M;
		default:               return -EINVAL;
		}
	}
}
...
{
	switch (id) {
	case MT8188_ETDM_IN1:  return MT8188_ETDM_SYNC_FROM_IN1;
	case MT8188_ETDM_IN2:  return MT8188_ETDM_SYNC_FROM_IN2;
	case MT8188_ETDM_OUT1: return MT8188_ETDM_SYNC_FROM_OUT1;
	case MT8188_ETDM_OUT2: return MT8188_ETDM_SYNC_FROM_OUT2;
	default:               return -EINVAL;
	}
}
 ...
 * -------------------------------------------------------------------------
 */
static void etdm_set_clock_source(uintptr_t base, enum mt8188_etdm_id id,
				  uint32_t source)
{
	uint32_t reg, mask;
 ...
 * -------------------------------------------------------------------------
 */
static int etdm_in_configure(uintptr_t base, enum mt8188_etdm_id id,
 ...
 * -------------------------------------------------------------------------
 */
static int etdm_out_configure(uintptr_t base, enum mt8188_etdm_id id,
			      uint32_t rate, uint32_t channels,
			      const struct mt8188_etdm_config *ecfg)
{
	const struct etdm_regs *r = &etdm_reg_table[id];
	...
	val  = FIELD_PREP(ETDM_OUT_CON4_RELATCH_EN_MASK,
			  (id == MT8188_ETDM_OUT1) ? MT8188_ETDM_OUT1_1X_EN
 ...
 * -------------------------------------------------------------------------
 */
static int etdm_sync_mode_slv(uintptr_t base, enum mt8188_etdm_id id,
			      int cowork_source_id, bool source_is_slave)
{
	...
	int cowork_source_sel = etdm_cowork_slv_sel(
		(enum mt8188_etdm_id)cowork_source_id, source_is_slave);
 ...
 * -------------------------------------------------------------------------
 */
static int etdm_sync_mode_mst(uintptr_t base, enum mt8188_etdm_id id,
				    ...
				    const struct mt8188_etdm_config *ecfg)
{
	...
	if (ecfg->slave_mode) {
		 ...
		 */
		return etdm_sync_mode_slv(afe->base, id, src,
					  afe->etdm[src].slave_mode);
	} else {
...
int mt8188_etdm_enable_mclk(struct mt8188_afe *afe, enum mt8188_etdm_id id)
{
	...
	if (ecfg->mclk_freq == 0) {
		...
		if (ecfg->mclk_dir == MT8188_MCLK_DIR_IN) {
			reg_update_bits(afe->base, etdm_reg_table[id].con1,
					ETDM_CON1_MCLK_OUTPUT, 0);
		}
		return 0;
	}
			  ...
			  const struct mt8188_afe_cfg *cfg)
{
	...
	etdm_channels = (ecfg->data_mode == MT8188_ETDM_DATA_ONE_PIN)
	...
	for (i = 0; i < ecfg->cowork_slv_count; i++) {
		...
		if (slv_id == MT8188_ETDM_IN1 || slv_id == MT8188_ETDM_IN2) {
			etdm_in_configure(afe->base, (enum mt8188_etdm_id)slv_id,
					  cfg->rate, channels, slv_ecfg);
		} else {
			etdm_out_configure(afe->base, (enum mt8188_etdm_id)slv_id,
					   cfg->rate, channels, slv_ecfg);
		}

		etdm_set_clock_source(afe->base, (enum mt8188_etdm_id)slv_id, 1);
		etdm_sync_mode_configure(afe, (enum mt8188_etdm_id)slv_id,
					 slv_ecfg);
	}
		     ...
		     enum mt8188_route_src src, enum mt8188_route_dst dst)
{
	...
	if ((unsigned int)src >= MT8188_ROUTE_SRC_NR ||
	    (unsigned int)dst >= MT8188_ROUTE_DST_NR) {
		return -EINVAL;
	}

#define ROUTE_TBL(slot, arr) do {			\
		tbl[slot]   = (arr);			\
...
#undef ROUTE_TBL
	...
	for (t = 0; t < ARRAY_SIZE(tbl); t++) {
		for (i = 0; i < tbl_n[t]; i++) {
			sys_write32(sys_read32(afe->base + tbl[t][i].reg) |
				    BIT(tbl[t][i].bit),
