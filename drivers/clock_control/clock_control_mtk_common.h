...
#define MAKE_CLK_MAP(_set, _clr, _status, _mask)                                                   \
	{                                                                                          \
		.set_offs = (uint32_t)_set, .clr_offs = (uint32_t)_clr,                            \
		.status_offs = (uint32_t)_status, .bit_mask = (uint32_t)_mask                      \
	}
...
extern int clk_ctrl_mtk_clock_on(const struct device *dev, clock_control_subsys_t sys);
extern int clk_ctrl_mtk_clock_off(const struct device *dev, clock_control_subsys_t sys);
extern enum clock_control_status clk_ctrl_mtk_clock_status(const struct device *dev,
							   clock_control_subsys_t sys);

extern int clk_ctrl_mtk_init(const struct device *dev);
